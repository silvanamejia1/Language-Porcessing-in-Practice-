...
### Methodology
**1. Curating Travel Database → **The first step to construct the agent was to gather sources that would inform the chatbot. These documents were gathered from trusted and diverse travel resources to have sufficient information that caters to the needs of different travelers. In order to do this, the main focus was on sources that recommended destinations, as the starting point often feels like the hardest decision to make. To do this, I included general recommendations like articles recommending top travel destinations for 2026 as well as destinations ideal for specific seasons, different types of travelers (luxury, backpacking, foodies, group trips), while always seeking to have geographic diversity (including recommendations from every continent). Below is the full list of documents that were used. Each document is then broken into smaller text chunks so that the system can retrieve specific, relevant passages instead of entire articles.
